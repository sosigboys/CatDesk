...
CatDesk — мой проект, который вырос в полноценный продукт. Началось с того, что надоели лимиты AnyDesk (2 часа и отключение), а RuDesktop просил деньги за каждую функцию. Решил сделать своё.

За два месяца написал удалённый рабочий стол с нуля: сигналинг-сервер на Node.js, десктопный клиент на Electron, P2P-видео через WebRTC. Сервер крутится на российском VPS за 300 рублей в месяц. Данные не ходят за границу.
...
Сознательно не использовал React/Vue. Хотел понять, как всё работает на низком уровне: DOM, события, WebSocket, WebRTC. Да, кода больше. Но зато знаю каждый байт.
